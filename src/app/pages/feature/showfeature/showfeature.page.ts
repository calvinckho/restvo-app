...
export class ShowfeaturePage implements OnInit, OnDestroy {
  ...
  newCalendarItem = {
    ...
    options: {
      // the details of the repeating schedule
  ...
  async ngOnInit() {
    ...
    if (this.relationshipId) {
      // if relationship context is provided, also assign it to the responseObj property
    ...
    this.subscriptions[
      "refreshMoment"
    ] = this.momentService.refreshMoment$.subscribe(this.refreshMomentHandler);
    // link refreshUserStatus observable with the loadMoment handler. It fires on page loads and subsequent user status refresh
    this.subscriptions[
      "refreshUserStatus"
    ] = this.userData.refreshUserStatus$.subscribe(
      this.loadAndProcessMomentHandler
    );
  ...
  loadAndProcessMomentHandler = async (data) => {
    ...
    if (this.moment._id) {
      // if called by modalCtrl.create()
      await this.loadMoment();
    } else {
      // if called by router outlet
      this.moment._id =
        this.route.snapshot.paramMap.get("id") || "5d5785b462489003817fee18";
      await this.loadMoment();
    }
    this.categoryIds = this.moment.categories
      ? this.moment.categories.map((c) => c._id)
      : [];
    ...
    if (
      (this.authService.token &&
        !this.token &&
        !this.hasAddedToAttendEventList &&
        !this.hasOrganizerAccess &&
        !this.hasLeaderAccess) ||
      this.token
    ) {
    ...
    if (res && res.momentId && res.data) {
      ...
      if (momentId === this.relationshipId) {
        if (data.calendarId) {
          ...
          const index = this.parentRelationshipResponseObj.matrix_string
            .map((c) => c[0])
            .indexOf(data.calendarId);
          if (index >= 0) {
            this.parentRelationshipResponseObj.matrix_string.splice(
              index,
              1,
              data.interactable
            );
          } else {
            this.parentRelationshipResponseObj.matrix_string.push(
              data.interactable
            );
          }
        } else if (data.goal) {
          if (data.action === "delete") {
            ...
            const i = this.parentRelationshipResponseObj.matrix_string
              .map((c) => c[0])
              .indexOf(data.goal[0]);
            if (i >= 0) {
              this.parentRelationshipResponseObj.matrix_string.splice(
        ...
        this.moment._id === momentId ||
        this.moment._id + (this.relationshipId || "") === momentId ||
        (this.calendarId || "") === momentId
      ) {
        if (data.response) {
          const index = this.responses
            .map((c) => c._id)
            .indexOf(data.response._id);
          if (index < 0) {
            // if the response hasn't been added to the response list
            this.responses.push(data.response);
          } else {
            // if it has been added, replace with the incoming one
          ...
          if (data.response.matrix_number) {
            for (const interactable of data.response.matrix_number) {
              ...
              if (this.interactableDisplay.hasOwnProperty(interactableId)) {
                ...
                const componentId = this.moment.resource.matrix_number[0][
                  this.moment.resource.matrix_number[2].indexOf(interactableId)
                ];
                if (componentId === 30000) {
                  // reset vote count to zero
                  this.interactableDisplay[interactableId].forEach(
                    (displayItem) => {
                      displayItem.count = 0;
                  ...
                  this.setupPollDisplay(
                    interactableId,
                    this.moment.resource.matrix_number[2].indexOf(
                      interactableId
                    )
                  );
                } else if (componentId === 40000) {
                  // multiple choice
                  this.setupInteractableDisplay(
                    interactableId,
                    this.moment.resource.matrix_number[2].indexOf(
                      interactableId
                    )
                  );
                } else if (componentId === 40010) {
                  // text answer
                  // using delta. see below
                } else if (componentId === 40020) {
                  // tile choice
                  this.setupInteractableDisplay(
                    interactableId,
                    this.moment.resource.matrix_number[2].indexOf(
          ...
          if (this.hasOrganizerAccess) {
            // only if it has Organizer Access do we load all content calendars from backend. this is for the event when a Community/Program super admin needs to access the calendar contents
            const results: any = await this.calendarService.loadRelationshipContentCalendars(
              this.moment._id
            );
          ...
          data.delta &&
          data.interactableId &&
          this.interactableDisplay.hasOwnProperty(data.interactableId)
        ) {
          // update the interactable Display (private or collaborative view)
          if (
            this.interactableDisplay.hasOwnProperty(data.interactableId) &&
            this.interactableDisplay[data.interactableId].editor
          ) {
            this.interactableDisplay[data.interactableId].editor.updateContents(
              data.delta,
              "silent"
            );
          }
          ...
          for (const calendarItem of this.calendarService.calendarItems) {
            if (calendarItem._id === data.calendarId) {
              // interactable[0] is a String
              if (data.state) {
          ...
          let index = this.responseObj.matrix_string
            .map((c) => c[0])
            .indexOf(data.calendarId);
          if (index >= 0) {
          ...
          if (data.action === "delete") {
            let index = this.listOfDisplayGoals
              .map((c) => c[0])
              .indexOf(data.goal[0]);
            if (index >= 0) {
            ...
            index = this.responseObj.matrix_string
              .map((c) => c[0])
              .indexOf(data.goal[0]);
            if (index >= 0) {
            ...
            const i = this.responseObj.matrix_string
              .map((c) => c[0])
              .indexOf(data.goal[0]);
            if (i >= 0) {
  ...
  async loadCalendarItem() {
    if (this.authService.token && this.calendarId) {
      this.calendarItem = await this.momentService.touchContentCalendarItems(
        null,
        { operation: "load calendar item", calendarId: this.calendarId }
      );
      if (this.calendarItem && this.calendarItem.uniqueAnswersPerCalendar) {
        // if calendar context is provided and if it is an unique answer per calendar, also assign it to the responseObj property
        this.responseObj.calendar = this.calendarId;
      }
      const results: any = await this.responseService.findResponsesByMomentId(
        this.relationshipId,
        null,
        null
      );
      if (results && results.responses && results.responses.length) {
        this.parentRelationshipResponseObj = JSON.parse(
          JSON.stringify(results.responses[results.responses.length - 1])
        );
        delete this.parentRelationshipResponseObj._id; // the latest response id is erased, since it may be saved as a new doc
        this.parentRelationshipListOfGoals = results.responses[
          results.responses.length - 1
        ].matrix_string.filter((c) => c[1] === "goal"); // goals and not master goals
        const parentRelationshipInteractable = results.responses[
          results.responses.length - 1
        ].matrix_string.find((c) => c[0] === this.calendarId);
        if (parentRelationshipInteractable) {
          this.parentRelationshipGoalAttributes = parentRelationshipInteractable.slice(
            10
          );
        }
      }
  ...
  refreshCalendarDisplay() {
    ...
    for (const calendarItem of this.adminAccessContentCalendars) {
      ...
      if (
        new Date(calendarItem.startDate).getTime() <
    ...
    for (const response of this.responses) {
      for (const interactable of response.matrix_string) {
        // process the interactable and schedule responses
        // regular user calendar items list
        for (const calendarItem of this.calendarService.calendarItems) {
          if (calendarItem._id === interactable[0] && interactable.length > 5) {
            // interactable[0] is a String
            calendarItem.completed = interactable[5]; // check-in state
          }
          if (
            calendarItem._id === interactable[0] &&
            interactable.length > 10
          ) {
            // interactable[0] is a String
        ...
        const index = this.responseObj.matrix_string
          .map((c) => c[0])
          .indexOf(interactable[0]);
        if (index >= 0) {
  ...
  async loadMoment() {
    ...
    if (this.moment) {
      ...
      if (
        ...
        this.moment.location.geo &&
        this.moment.location.geo.coordinates &&
        this.moment.location.geo.coordinates.length
      ) {
        this.mapURL =
          "https://maps.locationiq.com/v2/staticmap?key=pk.e5797fe100f9aa5732d5346f742b243f&center=" +
          this.moment.location.geo.coordinates[1] +
          "," +
          this.moment.location.geo.coordinates[0] +
          "&zoom=12&size=1000x600&maptype=roadmap&markers=icon:%20large-red-cutout%20|" +
          this.moment.location.geo.coordinates[1] +
          "," +
          this.moment.location.geo.coordinates[0];
        this.addressURL =
      ...
      if (
        this.moment.resource.matrix_number &&
        this.moment.resource.matrix_number.length
      ) {
        // if there is any interactable (schedule, poll, m.c., text answers), load responses
        if (
          this.moment.resource.matrix_number[0].find(
            (c) => c === 10210 || (c >= 30000 && c <= 49999)
          )
        ) {
          const results: any = await this.responseService.findResponsesByMomentId(
            this.moment._id,
            this.relationshipId,
            this.calendarItem && this.calendarItem.uniqueAnswersPerCalendar
              ? this.calendarId
              : null
          );
          if (results && results.responses) {
            if (this.responseId) {
              this.responses = results.responses.filter(
                (c) => c._id === this.responseId
              );
            } else {
          ...
          if (results && results.hasOrganizerLeaderAccess) {
            this.hasOrganizerAccess =
              this.hasOrganizerAccess || results.hasOrganizerLeaderAccess;
          }
          for (const response of this.responses) {
        ...
        if (this.moment.resource.matrix_number[0].find((c) => c === 10210)) {
          const schedules: any = await this.momentService.loadActivitySchedules(
            this.moment._id
          );
          this.customSchedule = schedules.find(
            (c) => c.options && !c.options.recurrence
          ); // customSchedule is a schedule with options.recurrence set to 'none'
          this.scheduleIds = schedules.map((c) => c._id);
          if (!this.hasOrganizerAccess) {
            // adminAccessContentCalendars is used instead of calendarService.calendarItems, so no need to update user's calendar for Organizer
        ...
        if (this.moment.resource.matrix_number[0].find((c) => c === 10500)) {
          ...
          this.participantsView =
            viewMap[
              this.moment.matrix_number[
                this.moment.resource.matrix_number[0].indexOf(10500)
              ].indexOf(1) || 0
            ];
        }
        ...
        if (this.moment.resource.matrix_number[0].find((c) => c === 20020)) {
          if (
            !this.tabSelection &&
            this.moment.matrix_number[
              this.moment.resource.matrix_number[0].indexOf(20020)
            ].length > 5
          ) {
            // assuming at least 1 tab has been created. if no default has been selected, use the first available tab
            this.tabSelection = JSON.parse(
              JSON.stringify(
                this.moment.matrix_number[
                  this.moment.resource.matrix_number[0].indexOf(20020)
                ][0] ||
        ...
        this.moment.resource.matrix_number[0].forEach(
          (componentId, componentIndex) => {
            if (this.moment.resource.matrix_number[2]) {
              // interactableId is a Number
              const interactableId = this.moment.resource.matrix_number[2][
                componentIndex
              ]; // if interactable, grab the createdAt timestamp
              if (componentId === 30000) {
                // poll
                this.interactableDisplay[interactableId] = [];
                for (const option of this.moment.matrix_string[
                  componentIndex
                ]) {
                  // set up the poll options
                  this.interactableDisplay[interactableId].push({
                    option: option,
                    count: 0,
                    votedByUser: false,
                  });
                ...
                for (const option of this.moment.matrix_string[
                  ...
                  // set up the interactable options
                  this.interactableDisplay[interactableId].push({
                    option: option,
                ...
                // text answer. Note: Collaborative Goals require updating this.responseObj with the latestResponse data
                this.interactableDisplay[interactableId] = { editor: null };
                // first determine if it is collaborative or private
                const isCollaborative =
                  this.moment.matrix_number[componentIndex].length > 1 &&
                  this.moment.matrix_number[componentIndex][1];
                this.interactableDisplay[
                  interactableId
                ...
                for (const interactable of latestResponse.matrix_string) {
                  // process the text answer responses
                  if (interactableId.toString() === interactable[0]) {
                    // interactableId is a Number
                    let content: any;
                    if (interactable.length > 2 && interactable[2]) {
                      ...
                      // for backward compatibility before quill, need to build the content object from scratch
                    ...
                    for (let responseInteractable of this.responseObj
                      .matrix_string) {
                      if (
                        responseInteractable[0] === interactableId.toString()
                      ) {
                        // InteractableId is in Number
                ...
                // tile choice
                this.interactableDisplay[interactableId] = {};
                this.moment.matrix_string[componentIndex].forEach(
                  (option, option_index) => {
                    // set up the interactable options
                    if (option_index % 2 === 0) {
                      // only even number, i.e. 0, 2, 4, 6
                      this.interactableDisplay[interactableId][
                        this.moment.matrix_number[componentIndex][
                          option_index + 5
                        ].toString()
                      ] = {
                        createdAt: this.moment.matrix_number[componentIndex][
                          option_index + 5
                        ],
                        option: option,
                        image: this.moment.matrix_string[componentIndex][
                          option_index + 1
                        ],
                        selectedByUser: false,
                      };
                ...
                this.setupInteractableDisplay(interactableId, componentIndex);
              } else if (componentId === 10210) {
                // goal attributes. Note: Collaborative Goals require updating this.responseObj with the latestResponse data
                ...
                if (this.responses.length) {
                  ...
                  this.listOfDisplayGoals = latestResponse.matrix_string.filter(
                    (c) => ["goal", "master goal"].includes(c[1])
                  );
                  // update this.responseObj with the latest goals data
                  this.responseObj.matrix_string = this.responseObj.matrix_string.filter(
                    (c) => !["goal", "master goal"].includes(c[1])
                  );
                  this.responseObj.matrix_string.push(
                    ...latestResponse.matrix_string.filter((c) =>
                      ["goal", "master goal"].includes(c[1])
                    )
                  );
                }
            ...
            if (
              componentId === 50000 &&
              this.moment.matrix_number[componentIndex][3] === 2
            ) {
              get(
                "https://api.tiles.mapbox.com/mapbox-gl-js/v1.2.1/mapbox-gl.js",
                () => {
                  get(
                    "https://tiles.unwiredmaps.com/v2/js/unwired-gl.js?v=0.1.6",
                    () => {
  ...
  async setupPermission() {
    if (this.momentService.socket) {
      if (
        ...
        this.calendarId &&
        this.calendarItem &&
        this.calendarItem.uniqueAnswersPerCalendar
      ) {
        ...
        this.momentService.socket.emit(
          "join moment",
          this.moment._id + this.relationshipId
        );
      } else {
        this.momentService.socket.emit("join moment", this.moment._id);
      }
    }
    const calendarItemIds = this.calendarService.calendarItems.map(
      (c) => c._id
    );
    this.hasAddedToCalendar =
      this.moment.calendar &&
      calendarItemIds.indexOf(this.moment.calendar._id) > -1;
    if (this.moment.user_list_1) {
      ...
      if (
        ...
        peopleComponentId = this.moment.resource.matrix_number[0].indexOf(
          10500
        );
      }
      if (peopleComponentId > -1) {
        this.participantLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 2 &&
          this.moment.matrix_string[peopleComponentId][2]
            ? this.moment.matrix_string[peopleComponentId][2]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][4];
        this.organizerLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 4 &&
          this.moment.matrix_string[peopleComponentId][4]
            ? this.moment.matrix_string[peopleComponentId][4]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][6];
        this.leaderLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 0 &&
          this.moment.matrix_string[peopleComponentId][0]
            ? this.moment.matrix_string[peopleComponentId][0]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][8];
        this.participantsLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 3 &&
          this.moment.matrix_string[peopleComponentId][3]
            ? this.moment.matrix_string[peopleComponentId][3]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][5];
        this.organizersLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 5 &&
          this.moment.matrix_string[peopleComponentId][5]
            ? this.moment.matrix_string[peopleComponentId][5]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][7];
        this.leadersLabel =
          this.moment.matrix_string[peopleComponentId].length &&
          this.moment.matrix_string[peopleComponentId].length > 1 &&
          this.moment.matrix_string[peopleComponentId][1]
            ? this.moment.matrix_string[peopleComponentId][1]
            : this.moment.resource["en-US"].matrix_string[peopleComponentId][9];
      }
      ...
      this.joinDisabled =
        this.moment.categories
          .map((c) => c._id)
          .includes("5e1bbda67b00ea76b75e5a73") ||
        this.moment.categories
          .map((c) => c._id)
          .includes("5e17acd47b00ea76b75e5a71");
      this.hasAddedToAttendEventList = this.moment.user_list_1
        .map((c) => c._id)
        .includes(this.userData.user._id);
      // if Activity's organizer
      if (
        this.moment.user_list_2
          .map((c) => c._id)
          .includes(this.userData.user._id)
      ) {
        ...
        ["owner", "admin", "staff"].includes(this.userData.user.role)
      ) {
        this.hasOrganizerAccess = true;
        // if the Activity has a parent Program and respective grandparent programs, also check their organizers list
      } else if (
        this.moment.parent_programs &&
        this.moment.parent_programs.length
      ) {
        const promises = this.moment.parent_programs.map(
          async (parent_program) => {
            if (
              parent_program.user_list_2 &&
              parent_program.user_list_2.includes(this.userData.user._id)
            ) {
              this.hasOrganizerAccess = true;
            }
            if (
              parent_program.parent_programs &&
              parent_program.parent_programs.length
            ) {
              const promises2 = parent_program.parent_programs.map(
                async (grandparent_program) => {
                  if (
                    grandparent_program.user_list_2 &&
                    grandparent_program.user_list_2.includes(
                      this.userData.user._id
                    )
                  ) {
                    this.hasOrganizerAccess = true;
                  }
      ...
      this.hasLeaderAccess = this.moment.user_list_3
        .map((c) => c._id)
        .includes(this.userData.user._id);
      this.setupPermissionCompleted = true;
    }
    ...
    for (const response of this.responses) {
      for (const interactable of response.matrix_number) {
        if (interactableId === interactable[0]) {
          ...
          if (
            interactable[1] >
            this.moment.matrix_string[componentIndex].length - 1
          ) {
  ...
  async setupInteractableDisplay(interactableId, componentIndex) {
    for (const response of this.responses) {
      for (const interactable of response.matrix_number) {
        // process the interactable responses
        if (interactableId === interactable[0]) {
          if (
            interactable.length - 5 >
            this.moment.matrix_string[componentIndex].length
          ) {
            return; // if this response belongs to an option that has been deleted
          }
          if (response.user._id === this.userData.user._id) {
            for (let i = 5; i < interactable.length; i++) {
              if (
                this.moment.resource.matrix_number[0][componentIndex] === 40020
              ) {
                // for tile choice, the response records the user's choice using the option's createdAt timestamp [200819285102, 201928523192, etc]
                this.interactableDisplay[interactableId][
                  interactable[i].toString()
                ].selectedByUser = true;
              } else {
                // for older interactables (e.g. multiple choice), response records the user's choice using the option index [1, 5, 4]
                this.interactableDisplay[interactableId][
                  interactable[i]
                ].selectedByUser = true;
              }
  ...
  async addToCalendar(listOfConversations, listOfUsers) {
    try {
      const result: any = await this.momentService.updateMomentUserLists(
        ...
        this.token
      ); // a valid token is not required, but provided in case of future change of specs
  ...
  async removeFromUserList(user_list) {
    try {
      if (this.networkService.hasNetwork) {
        ...
        if (this.moment.conversation) {
          const count = await this.chatService.resetBadgeCount(
            this.moment.conversation
          );
          if (count) {
            if (
              this.platform.is("cordova") &&
              this.userData.user.enablePushNotification
            ) {
              this.badge.decrease(count);
            }
            if (this.electronService.isElectronApp) {
              this.electronService.ipcRenderer.send(
                "SYSTEM_TRAY:::CHANGE_BADGE",
                -1 * count
              );
            }
          }
          ...
          if (!this.modalPage) {
            this.userData.refreshUserStatus({
              type: "close group view",
              data: { _id: this.moment.conversation },
            });
          }
          this.chatService.socket.emit(
            "leave conversation",
            this.moment.conversation
          ); // inform the socket.io server this user is leaving this room
          this.userData.refreshMyConversations({
            action: "disconnect chat view",
    ...
    console.log(this.userData.defaultProgram._id, this.moment._id);
    if (this.userData.defaultProgram._id === this.moment._id) {
      console.log("theya re the same");
      ...
      const activity = newActivities.find(
        (n) => n._id === "5d5785b462489003817fee18"
      ); // finding an Activity that is not Restvo Mentor);
      console.log("activity", activity);
      if (activity) {
        console.log("activity is here", activity);
        this.userData.defaultProgram = activity;
        this.userData.UIAdminMode = false; // toggling on the Mentoring Mode
      ...
      if (this.hasOrganizerAccess) {
        const modal = await this.modalCtrl.create({
          ...
          componentProps: {
            moment: this.moment,
            title:
              this.resource["en-US"].value[32] +
              " to " +
              this.moment.matrix_string[0][0],
            modalPage: true,
          },
        });
        await modal.present();
      } else {
        this.momentService.addParticipants(
          this.moment,
          this.resource,
          "both",
          ["user_list_1"],
  ...
  async editMoment(advanced) {
    if (this.modalPage) {
      let modal;
      if (
        this.moment.categories.find(
          (c) => c._id === "5c915324e172e4e64590e346"
        ) &&
        !advanced
      ) {
        modal = await this.modalCtrl.create({
          component: CreatefeaturePage,
          componentProps: { moment: this.moment, modalPage: true },
        });
      } else {
        modal = await this.modalCtrl.create({
  ...
  async deleteMoment() {
    const alert = await this.alertCtrl.create({
      header:
        this.resource["en-US"].value[19] +
        " " +
        this.moment.resource["en-US"].value[0],
      message:
        this.resource["en-US"].value[22] +
        " " +
        this.moment.matrix_string[0][0] +
        "? " +
        (this.moment.resource.matrix_number &&
        this.moment.resource.matrix_number.length &&
        this.moment.resource.matrix_number[0].indexOf(10370) > -1
          ? this.resource["en-US"].value[23]
          : ""),
      buttons: [
        {
          text: "Ok",
  ...
  async seeUserInfo(event, user) {
    ...
    if (!this.authService.token) {
      ...
      this.userData.refreshUserStatus({
        type: "show recipient",
        data: { recipient: user, modalPage: true },
      });
    }
  }
  ...
  async respondToInteractable(event, componentIndex, interactableOption) {
    ...
    for (let interactable of this.responseObj.matrix_number) {
      // each interactable is an array of user choices for each question
      if (interactable[0] === interactableId) {
        for (let i = interactable.length - 1; i > 4; i--) {
          // for multiple choice or tile choice
          if (interactable[i] === interactableOption) {
            // de-select an existing selection
        ...
        if (!updatedExistingResponse) {
          ...
          if (this.moment.matrix_number[componentIndex][0]) {
            // if multiple selection is on
            maxNumOfSelections =
              this.moment.matrix_number[componentIndex][1] ||
              this.moment.matrix_string[componentIndex].length; // if max number of selections has not been set (null), use the number of available options
          }
          while (interactable.length - 5 > maxNumOfSelections) {
            // splice the oldest selection until it satisfies the max num of selections
    ...
    if (!updatedExistingResponse) {
      // add a new entry to array
      this.responseObj.matrix_number.push([
        interactableId,
        null,
        null,
        null,
    ...
    let response = await this.momentService.submitResponse(
      this.moment,
      this.responseObj,
      false
    );
  ...
  async touchCalendarItemResponse(event, selectedCalendarItem, type) {
    ...
    let updatedExistingResponse = false;
    this.responseObj.matrix_string.forEach((interactable) => {
      // each interactable is an array of user choices for each question
      if (interactable[0] === selectedCalendarItem._id) {
        // interactable[0] is a String
        if (type === "check-in") {
    ...
    if (type === "check-in") {
      ...
      for (const calendarItem of this.calendarService.calendarItems) {
        if (calendarItem._id === selectedCalendarItem._id) {
          // interactable[0] is a String
    ...
    const response = await this.momentService.submitResponse(
      this.moment,
      this.responseObj,
      false
    );
    const socketData: any = {
      ...
      interactable: this.responseObj.matrix_string.find(
        (c) => c[0] === selectedCalendarItem._id
      ),
      author: {
    ...
    this.momentService.socket.emit(
      ...
      this.moment._id,
      socketData
    ); // Using the moment service socket.io to signal real time dynamic update for other users in the same momentId room
  ...
  async createQuillEditor(event, interactableDisplay) {
    ...
    interactableDisplay.editor.setContents(
      interactableDisplay.content,
      "silent"
    );
  ...
  async checkAndLoadNotes() {
    ...
    if (this.moment.resource.matrix_number[0].find((c) => c === 20020)) {
      ...
      if (
        index >= 0 &&
        this.moment.resource.matrix_number[3][index] === this.tabSelection
      ) {
        // if default tab selection has selected the Notes view, check and load notes
        const result: any = await this.momentService.loadNotes(
          this.moment._id,
          null
        );
    ...
    this.notes.sort((a, b) => {
      // sort notes with the most recent first
  ...
  async respondToTextArea(event, componentIndex) {
    ...
    if (responseType) {
      // if response type is collaborative
      const socketData = {
        delta: event.delta,
      ...
      if (
        ...
        this.momentService.socket.emit(
          ...
          this.calendarId,
          socketData
        ); // Using the moment service socket.io to signal real time dynamic update for other devices in the same conversationId room
      } else if (this.relationshipId) {
        this.momentService.socket.emit(
          "refresh moment",
          this.moment._id + this.relationshipId,
    ...
    for (const interactable of this.responseObj.matrix_string) {
      if (
        interactable[0] === interactableId.toString() &&
        this.interactableDisplay[interactableId].editor
      ) {
        // InteractableId is in Number
        interactable[1] = event.text;
        interactable[2] = JSON.stringify(
          this.interactableDisplay[interactableId].editor.getContents()
  ...
  async loadPeople() {
    setTimeout(() => {
      if (this.authService.token && this.infiniteScroll) {
        // infinite scroll for 50000 match users only shows for authenticated users
  ...
  async loadMorePeople(event) {
    this.pageNum++;
    if (!this.reachedEnd && !this.loadAPIBusy) {
      // loadAPIBusy is used to safeguard against iOS calling the (ionInfiniteScroll) function from the DOM that races with the loadMorePeople function
      this.loadAPIBusy = true;
      setTimeout(() => {
        this.loadAPIBusy = false;
      }, 10000);
      const results: any = await this.momentService.loadMatchedPeople(
        this.moment._id || "",
        this.searchKeyword,
        this.pageNum
      );
  ...
  async loadMoreSearchResults(event) {
    ...
    if (!this.mapService.reachedEnd && !this.mapService.loadAPIBusy) {
      // loadAPIBusy is used to safeguard against iOS calling the (ionInfiniteScroll) function from the DOM that races with the loadMoreSearchResults function
      this.mapService.loadAPIBusy = true;
      this.mapService.searchRadius =
        (this.platform.width() *
          97.27157 *
          0.5 *
          Math.cos((this.mapService.userLocation.lat * Math.PI) / 180)) /
        Math.pow(2, this.mapService.userZoom);
      ...
      this.zone.run(async () => {
        const results: any = await this.momentService.loadNearbyPublicActivities(
          this.searchKeyword,
          searchType,
  ...
  async acceptInvitation(event) {
    ...
    if (this.authService.token && this.participant_type && this.token) {
      ...
      this.openRegister(
        0,
        "You need to sign in or create an account first before you can accept the invitation."
      );
    }
  }
...
>>>>>>> updated default program to be Restvo when user leaves the default group
  ...
  async openNote(note) {
    if (note.content_id) {
      ...
      componentProps = {
        moment: { _id: momentId },
        relationshipId: this.moment._id,
        modalPage: true,
      };
      ...
      if (this.platform.width() >= 768) {
        if (this.authService.token) {
          this.router.navigate(["/app/activity/" + momentId, params], {
            replaceUrl: false,
          });
        } else {
          // for unauthenticated user and in landing page view, which is not a common case
          this.router.navigate(["/activity/" + momentId], {
            replaceUrl: false,
          });
        }
      } else {
        if (this.authService.token) {
          // for authenticated user, use modalCtrl for in app experience
          const modal = await this.modalCtrl.create({
  ...
  async moreParticipantOptions() {
        ...
        ...[
          ...
          },
          {
            ...
            handler: () => {
              ...
              navTransition.then(async () => {
                if (this.modalPage) {
                  this.momentService.openPreferences({
                    programId: this.moment._id,
                    modalPage: true,
                  });
                } else {
                  this.router.navigate([
                    "/app/preferences/" + this.moment._id,
                    { showHeader: true },
                  ]);
                }
              });
    ...
    if (
      (this.hasAddedToAttendEventList ||
        this.hasLeaderAccess ||
        this.hasOrganizerAccess) &&
      this.moment.array_boolean &&
      this.moment.array_boolean.length &&
      this.moment.array_boolean.length > 5 &&
      this.moment.array_boolean[5]
    ) {
      buttons.push({
    ...
    if (
      ...
      !this.moment.categories
        .map((c) => c._id)
        .includes("5c915476e172e4e64590e349")
    ) {
  ...
  async moreOrganizerActions() {
    ...
    buttons = buttons.concat([
      {
        text:
          this.resource["en-US"].value[43] +
          " " +
          this.moment.resource["en-US"].value[0], // Manage Community or Program
        icon: "bulb",
        handler: () => {
          const navTransition = actionSheet.dismiss();
          navTransition.then(async () => {
            this.momentService.manageMoment({
              moment: this.moment,
    ...
    if (this.hasOrganizerAccess) {
      ...
      if (
        this.moment.array_boolean &&
        this.moment.array_boolean.length &&
        this.moment.array_boolean.length > 6 &&
        this.moment.array_boolean[6]
      ) {
        buttons.push({
          text: this.resource["en-US"].value[41], // View Chat
          icon: "chatbubbles",
  ...
  async moreLeaderOptions() {
    ...
    if (this.hasLeaderAccess) {
      // since organizer can also open this menu, needs to check for Leader's privilege
      buttons = buttons.concat([
        {
          text: this.resource["en-US"].value[35], // View Onboarding Answers
          icon: "list",
    ...
    if (
      (this.hasLeaderAccess || this.hasOrganizerAccess) &&
      this.moment.array_boolean &&
      this.moment.array_boolean.length &&
      this.moment.array_boolean.length > 7 &&
      this.moment.array_boolean[7]
    ) {
      buttons.push({
        text: this.resource["en-US"].value[41], // View Chat
        icon: "chatbubbles",
  ...
  async cloneMoment() {
    ...
    const clonedMoments: any = await this.momentService.clone(
      [momentToBeCloned],
      "staff"
    );
    if (clonedMoments && clonedMoments.length) {
      const networkAlert = await this.alertCtrl.create({
        header: "Success",
        message:
          "You have successfully cloned " +
          clonedMoments[0].matrix_string[0][0] +
          ". It will be available in the Manage Activities page in a moment.",
        buttons: ["Dismiss"],
        cssClass: "level-15",
      });
  ...
  async loadMorePrograms() {
    this.programPageNum++;
    if (!this.programsReachedEnd) {
      // loadAPIBusy is used to safeguard against iOS calling the (ionInfiniteScroll) function from the DOM that races with the loadMoreMentors function
      const results: any = await this.momentService.loadPublicActivityByCategory(
        "all",
        this.programPageNum
      );
      this.ionSpinner = false;
      if (!results.length) {
        ...
        for (const result of results) {
          if (this.moment._id !== result._id) {
            // do not show the current program in the list
  ...
  async openProgram(event, moment) {
    ...
    if (this.platform.width() >= 768) {
      if (this.authService.token) {
        ...
        this.router.navigate(["/activity/" + moment._id], {
          replaceUrl: false,
        });
      }
    } else {
      if (this.authService.token) {
        ...
        const modal = await this.modalCtrl.create({
          component: ShowfeaturePage,
          componentProps: { moment: { _id: moment._id }, modalPage: true },
        });
        await modal.present();
      } else {
        // for unauthenticated user and in landing page view, use angular router
        this.router.navigate(["/activity/" + moment._id], {
          replaceUrl: false,
        });
      }
  ...
  async openContent(event, calendarItem) {
    ...
    const momentId =
      typeof calendarItem.moment === "object"
        ? calendarItem.moment._id
        : calendarItem.moment;
    let params: any;
    let componentProps: any;
    params = { relationshipId: this.moment._id };
  ...
  async addGoal(goalName) {
    ...
    const goalData = [
      Math.floor((Math.random() + new Date().getTime()) * 1000).toString(),
      "goal",
      null,
      this.color_arrays.find((c) => !c.status).color,
      null,
      goalName,
    ];
  ...
  async moreGoalFunctions(event, goal, componentIndex) {
    event.stopPropagation();
    const masterGoalLabel =
      this.moment.matrix_string[componentIndex].length &&
      this.moment.matrix_string[componentIndex][0]
        ? this.moment.matrix_string[componentIndex][0]
        : this.resource["en-US"].matrix_string[
            this.resource.matrix_number[0].indexOf(10210)
          ][6];
    let buttons = [];
    buttons = buttons.concat([
      {
        text:
          goal[1] === "goal"
            ...
            : this.resource["en-US"].matrix_string[
                this.resource.matrix_number[0].indexOf(10210)
              ][11], // Set as Favorite Goal, Remove Favorite
        handler: () => {
    ...
    if (
      goal[1] === "goal" &&
      this.responseObj.matrix_string.filter((c) =>
        ["master goal"].includes(c[1])
      ).length
    ) {
      buttons = buttons.concat([
        {
          text:
            ...
            " " +
            masterGoalLabel, // Tag to a Favorite Goal
          handler: () => {
            const navTransition = actionSheet.dismiss();
            navTransition.then(async () => {
  ...
  getColumnOptions(columnIndex) {
    ...
    for (
      ...
      options.push({
        text: this.responseObj.matrix_string.filter((c) =>
          ["master goal"].includes(c[1])
        )[i][5], //.substring(0, 10) + (this.responseObj.matrix_string.filter((c) => ['master goal'].includes(c[1]))[i][5].length > 10) ? '...' : '',
        value: this.responseObj.matrix_string.filter((c) =>
          ["master goal"].includes(c[1])
        )[i],
      });
    }
  ...
  async createCalendarItem(event, momentId) {
    ...
    this.newCalendarItem.startDateObj = new Date(
      new Date().getFullYear(),
      new Date().getMonth(),
      new Date().getDate(),
      new Date().getHours()
    ); // cache datetime info for computation only. backend datetime format is in ISOString
    this.newCalendarItem.endDateObj = new Date(
      new Date().getFullYear(),
      new Date().getMonth(),
      new Date().getDate(),
      new Date().getHours()
    ); // cache datetime info for computation only. backend datetime format is in ISOString
    this.newCalendarItem.startDate = new Date(
      new Date().getFullYear(),
      new Date().getMonth(),
      new Date().getDate(),
      new Date().getHours()
    ).toISOString(); // cache datetime info for computation only. backend datetime format is in ISOString
    this.newCalendarItem.endDate = new Date(
      new Date().getFullYear(),
      new Date().getMonth(),
      new Date().getDate(),
      new Date().getHours()
    ).toISOString(); // cache datetime info for computation only. backend datetime format is in ISOString
    this.newCalendarItem.uniqueAnswersPerCalendar =
      this.customSchedule.array_boolean.length > 1 &&
      this.customSchedule.array_boolean[1];
    const createdContentCalendarItem: any = await this.momentService.touchContentCalendarItems(
      this.moment._id,
      { operation: "create calendar item", calendaritem: this.newCalendarItem }
    );
    ...
    if (momentId === "5e4b39cd76ffc96ae6aba323") {
      const responseObj: any = JSON.parse(
        JSON.stringify(this.responseTemplate)
      );
      ...
      await this.momentService.submitResponse(
        { _id: momentId },
        responseObj,
        false
      );
  ...
  async saveCalendarItem() {
    ...
    if (this.moment._id !== this.responseObj.moment) {
      // in the event that the this.moment._id has been changed to another object id
      // create a new Content Calendar
      this.calendarItem.moment = this.moment._id;
      const calendarItem: any = await this.momentService.touchContentCalendarItems(
        this.relationshipId,
        { operation: "create calendar item", calendaritem: this.calendarItem }
      );
      ...
      this.momentService.touchContentCalendarItems(this.relationshipId, {
        operation: "delete calendar items",
        calendaritems: [this.calendarItem],
      });
    } else {
      // for all other Content Calendar item save
      // just updating the calendar
      await this.momentService.touchContentCalendarItems(this.relationshipId, {
      ...
      const index = this.parentRelationshipResponseObj.matrix_string
        .map((c) => c[0])
        .indexOf(this.calendarId);
      if (index >= 0) {
        this.parentRelationshipResponseObj.matrix_string[index].splice(
          10,
          this.parentRelationshipResponseObj.matrix_string[index].length - 10
        );
        this.parentRelationshipResponseObj.matrix_string[index].push(
      ...
      await this.momentService.submitResponse(
        { _id: this.relationshipId },
        this.parentRelationshipResponseObj,
        false
      );
      let socketData: any;
      socketData = {
        calendarId: this.calendarId,
        interactable: this.parentRelationshipResponseObj.matrix_string.find(
          (c) => c[0] === this.calendarId
        ),
  ...
  changeCalendarItemSelectedDate(inputDate) {
    ...
    this.calendarItem.startDate = new Date(
      inputDate.getFullYear(),
      inputDate.getMonth(),
      inputDate.getDate(),
      new Date(this.calendarItem.startDate).getHours(),
      new Date(this.calendarItem.startDate).getMinutes()
    ).toISOString();
    this.anyChangeMade = true;
  }
  ...
  async focusPhoto(event, object) {
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: { imageUri: object },
    });
    await modal.present();
  }
  ...
  closeModal() {
    ...
    if (this.authService.token && this.modalPage) {
      ...
      if (this.relationshipId && this.anyChangeMade) {
        setTimeout(() => {
          this.momentService.refreshMoment({
            momentId: this.relationshipId,
  ...
  async processVerificationToken() {
    ...
    this.participant_type = this.route.snapshot.paramMap.get("type")
      ? parseInt(this.route.snapshot.paramMap.get("type"), 10)
      : null;
    if (this.verification_token) {
      try {
        ...
        if (result.success && result.token) {
          ...
          this.authService.cachedRouteParams = {
            type: this.participant_type,
            token: this.token,
          }; // reinstate the invitation params
          // this will reload the app by first validating the returned auth token
          const res: any = await this.authService.checkAuthenticationWithToken(
            result.token
          );
