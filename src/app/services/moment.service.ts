...
export class Moment {
  ...
  public readonly openMoment$: Observable<
    any
  > = this._openMoment.asObservable();
  public readonly editMoment$: Observable<
    any
  > = this._editMoment.asObservable();
  public readonly refreshMoment$: Observable<
    any
  > = this._refreshMoment.asObservable();
  public readonly manageMoment$: Observable<
    any
  > = this._manageMoment.asObservable();
  public readonly openPreferences$: Observable<
    any
  > = this._openPreferences.asObservable();
  public readonly editParticipants$: Observable<
    any
  > = this._editParticipants.asObservable();
  ...
  async createMomentSocket() {
    if (
      ...
      this.networkService.domain !== "https://server.restvo.com"
    ) {
  ...
  load(id) {
    return this.http
      .get(
        this.networkService.domain + "/api/moment/" + id,
  ...
  loadUserPreferences(pageNum, programId, type) {
    return this.http
      .get(
        this.networkService.domain +
          "/api/moment/preferences?pageNum=" +
          pageNum +
          "&programId=" +
          (programId || "") +
          "&type=" +
  ...
  loadSampleActivities(categoryId) {
    return this.http
      .get(
        this.networkService.domain +
          "/api/moment/samples?version=1&category=" +
          (categoryId || ""),
        this.authService.httpAuthOptions
      )
      .toPromise();
  ...
  loadOnboardingFlow(momentId, searchKeyword, pageNum) {
    // admin only - Manage Development page
    return this.http
      .get(
        this.networkService.domain +
          "/api/moment/onboarding?momentId=" +
          (momentId || "") +
          "&searchKeyword=" +
          searchKeyword +
          "&pageNum=" +
  ...
  loadPublicMoment(id) {
    return this.http
      .get(
        this.networkService.domain + "/api/moment/public/" + id,
  ...
  async create(moment) {
    ...
    const promise = await this.http
      .post(
        this.networkService.domain + "/api/moment/create",
        JSON.stringify(data),
        this.authService.httpAuthOptions
      )
      .toPromise();
  ...
  async clone(moments, optOutReason) {
    const promises = await this.http
      .put(
        this.networkService.domain + "/api/moment/clone",
        JSON.stringify({ moments: moments, optOutReason: optOutReason }),
        this.authService.httpAuthOptions
      )
      .toPromise();
    await this.calendarService.getUserCalendar();
  ...
  async update(moment) {
    ...
    const promise = await this.http
      .put(
        this.networkService.domain + "/api/moment/update",
        JSON.stringify(data),
        this.authService.httpAuthOptions
      )
      .toPromise();
  ...
  async adoptPlan(data) {
    const promise = await this.http
      .put(
        this.networkService.domain + "/api/moment/plan/adopt",
        JSON.stringify(data),
        this.authService.httpAuthOptions
      )
      .toPromise();
  ...
  async share(moment) {
    if (moment.conversations && moment.conversations.length) {
      // moment.conversations are chat rooms to be associated with this moment. this is to be distinguished from moment.conversation, which is moment's own chat room. consult models/moment.js for detail
      let result = "";
      if (moment.calendar && moment.calendar._id) {
        const data = {
          // add all user in chat to the moment's calendar
          operation: "add to calendar",
          user_lists: [],
          momentId: moment._id,
          conversations: moment.conversations.map(
            (obj) => obj.conversation._id
          ),
          calendarId: moment.calendar._id,
        };
        if (
          (moment.resource &&
            moment.resource.hasOwnProperty("en-US") &&
            moment.resource["en-US"].value[0] === "Goal") ||
          moment.resource["en-US"].value[0] === "Meetup"
        ) {
          ...
          const promises = moment.conversations.map(async (obj) => {
            if (obj.conversation.group) {
              await this.chatService.sendReply(
                obj.conversation._id,
                {
                  moment: moment._id,
                  page:
                    obj.conversation.type === "connect"
                      ? "MessagePage"
                      : "GroupmessagePage",
                  groupId:
                    obj.conversation.type === "connect"
                      ? null
                      : obj.conversation.group._id,
                  groupName:
                    obj.conversation.type === "connect"
                      ? null
  ...
  async updateMomentUserLists(data, token) {
    const promise = await this.http
      .put<string>(
        this.networkService.domain +
          "/api/moment/updatemomentuserlists?token=" +
          token,
        JSON.stringify(data),
        this.authService.httpAuthOptions
      )
      .toPromise();
    this.socket.emit("refresh moment", data.momentId, {
      type: "refresh participation",
    });
  ...
  async submitResponse(moment, serverData, enableSocketIO) {
    // need to keep this as a moment service to utilize the moment's socket.io object
    try {
  ...
  async addUserToProgramUserList(momentId, user_list, type, token, notifyUser) {
    ...
    if (this.userData.user) {
      try {
        ...
        if (notifyUser) {
          // open modal box to notify user of status of joining the program
          if (result === "success") {
            if (type <= 2) {
              // participant
              const alert = await this.alertCtrl.create({
                header: "Success",
                message:
                  "You have successfully joined " + moment.matrix_string[0][0],
                buttons: [
                  {
                    text: "Ok",
                    handler: () => {
              ...
              const alert = await this.alertCtrl.create({
                header: "Success",
                message:
                  "You have now been added as " +
                  (user_list === "user_list_2"
                    ? "an organizer of "
                    : user_list === "user_list_3"
                    ? "a leader of "
                    : "") +
                  moment.matrix_string[0][0],
                buttons: [
                  {
                    text: "Ok",
                    handler: () => {
            ...
            const alert = await this.alertCtrl.create({
              ...
              message:
                "You do not have the required permission. Please contact the organizer for assistance.",
              buttons: [
                {
                  text: "Dismiss",
                  handler: () => {
  ...
  async initiateParticipantsView(moment, loading) {
    ...
    if (
      moment.user_list_2 &&
      moment.user_list_2.length &&
      moment.user_list_2[0] &&
      typeof moment.user_list_2[0] === "object"
    ) {
      // if user_list is populated, i.e. array of objects
      hasOrganizerAccess =
        moment.user_list_2.map((c) => c._id).includes(this.userData.user._id) ||
        ["owner", "admin", "staff"].includes(this.userData.user.role);
    } else if (
      moment.user_list_2 &&
      moment.user_list_2.length &&
      moment.user_list_2[0] &&
      typeof moment.user_list_2[0] === "string"
    ) {
      // if user_list is not populated, i.e. array of strings
      hasOrganizerAccess =
        moment.user_list_2.includes(this.userData.user._id) ||
        ["owner", "admin", "staff"].includes(this.userData.user.role);
    }
    if (loading) {
    ...
    if (hasOrganizerAccess) {
      this.editParticipants({
        moment: moment,
        title:
          this.resourceService.resource["en-US"].value[32] +
          " to " +
          moment.matrix_string[0][0],
        modalPage: true,
      });
      ...
      if (peopleComponentId > -1) {
        participantsLabel =
          moment.matrix_string[peopleComponentId].length &&
          moment.matrix_string[peopleComponentId].length > 3 &&
          moment.matrix_string[peopleComponentId][3]
            ? moment.matrix_string[peopleComponentId][3]
            : moment.resource["en-US"].matrix_string[peopleComponentId][5];
      }
  ...
  async addParticipants(
    ...
    this.chatService.conversations.forEach((item) => {
      if (
        (item.conversation.type === "connect" ||
          item.conversation.type === "self") &&
        item.data.participant &&
        moment[listOfNames[0]]
          .map((c) => c._id)
          .indexOf(item.data.participant._id) > -1
      ) {
        item.locked = true;
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        moment: moment,
        invitationType: listOfNames[0],
        filter: filter,
        includeSelf: true,
        title: title,
        action: action,
        conversations: selectedPersonOrGroup,
      },
    });
    await modal.present();
    ...
    let userObjectIds = [];
    const conversations =
      result && result.conversations ? result.conversations : [];
    const listOfAppUsers =
      result && result.listOfAppUsers ? result.listOfAppUsers : [];
    ...
    if (response === "success") {
      // only if the users are successfully added do we prepare to send out notifications
      ...
      if (listOfAppUsers && listOfAppUsers.length) {
        const promises = listOfAppUsers.map(async (appUser) => {
          ...
          const isConnected: any = await this.chatService.getConversationByRecipientId(
            appUser._id,
            false,
            false
          ); // API-controlled recipient info access permission
          if (isConnected && isConnected.conversation) {
            // if the recipient has been connected
            ...
            const welcomeMessage =
              this.userData.user.first_name +
              " " +
              this.userData.user.last_name +
              " is now connected with you.";
            const newConversationId = await this.chatService.newConversation(
              appUser._id,
              { composedMessage: welcomeMessage, type: "connect" }
            );
            conversations.push({
              conversation: { _id: newConversationId, type: "connect" },
            });
          }
        });
        await Promise.all(promises);
      ...
      this.chatService.notifyOfInvitation(
        conversations,
        moment,
        listOfNames[0]
      );
  ...
  async delete(moment) {
    const promise = await this.http
      .delete(
        this.networkService.domain + "/api/moment/" + moment._id,
        this.authService.httpAuthOptions
      )
      .toPromise();
    await this.calendarService.getUserCalendar();
  ...
  async loadSchedule(scheduleId) {
    return await this.http
      .get(
        this.networkService.domain + "/api/moment/schedule/" + scheduleId,
        this.authService.httpAuthOptions
      )
      .toPromise();
  }
  ...
  async loadProgramInsight(programId) {
    return await this.http
      .get(
        this.networkService.domain + "/api/moment/program/insight/" + programId,
        this.authService.httpAuthOptions
      )
      .toPromise();
  }
  ...
  async touchSchedule(data) {
    ...
    if (
      ...
      schedule.child_moments.length &&
      typeof schedule.child_moments[0] === "object"
    ) {
      // depopulate child_moments before send them back to server
      schedule.child_moments = schedule.child_moments.map((c) => c._id);
    }
    if (
      schedule.options &&
      schedule.options.hasOwnProperty("recurrence") &&
      schedule.options.recurrence === ""
    ) {
      delete schedule.options.recurrence;
    }
    const promise = await this.http
      .put(
        this.networkService.domain + "/api/moment/schedule/touch",
  ...
  async touchContentCalendarItems(momentId, data) {
    const promise = await this.http
      .put(
        this.networkService.domain + "/api/moment/contentcalendaritems/touch",
        JSON.stringify(data),
        this.authService.httpAuthOptions
      )
      .toPromise();
